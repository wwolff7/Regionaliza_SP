        ...
        if (this.Get('chart.tickmarks.small.num')) {
            for (var i=0; i<PI; i+=(PI / this.Get('chart.tickmarks.small.num'))) {
                this.context.beginPath();
                    ...
                    this.context.arc(this.centerx, this.centery, this.radius, PI + i, PI + i + 0.00001, 0);
        ...
        if (this.Get('chart.tickmarks.big.num')) {
            ...
            for (var j=0; j<colors.length; ++j) {
                for (var i=0; i<PI; i+=(PI / this.Get('chart.tickmarks.big.num'))) {
                    ...
                    this.context.arc(this.centerx, this.centery, this.radius, PI + i, PI + i + 0.001, 0);
        ...
        if (RGraph.is_array(this.Get('chart.colors.ranges'))) {
            ...
            if (this.Get('chart.segment.radius.start') > 0) {
                this.context.arc(this.centerx,
                                 this.centery,
                                 this.Get('chart.segment.radius.start'),
                                 (((this.Get('chart.yellow.end') - this.min) / (this.max - this.min)) * PI) + PI,
                                 (((this.Get('chart.yellow.start') - this.min) / (this.max - this.min)) * PI) + PI,
                                 true);
            } else {
                this.context.lineTo(this.centerx, this.centery);
            }
            ...
            this.context.beginPath();
                ...
                if (this.Get('chart.segment.radius.start') > 0) {
                    this.context.arc(this.centerx,
                                     this.centery,
                                     this.Get('chart.segment.radius.start'),
                                     (((this.Get('chart.red.end') - this.min) / (this.max - this.min)) * PI) + PI,
                                     (((this.Get('chart.red.start') - this.min) / (this.max - this.min)) * PI) + PI,
                                     true);
                } else {
                    this.context.lineTo(this.centerx, this.centery);
                }
